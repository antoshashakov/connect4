        ...
        for i in range(move_limit):
            ...
            for k in range(7 * trials):
                ...
                if (result == 1 and board_list[k].player_turn == player) or (
                    ...
                    stats[k // trials][0] += large_increment // moves_made
                    finished[k] = True
                if (result == 1 and board_list[k].player_turn != player) or (
                        result == -1 and board_list[k].player_turn == player):
                    stats[k // trials][1] += large_increment // moves_made
                    finished[k] = True
        ...
        for j in range(length):
            ...
            for k in range(7 * trials):
                ...
                if (result == 1 and board_list[(j * trials * 7) + k].player_turn == player[j]) or (
                    ...
                    stats[j][k // trials][0] += large_increment // moves_made
                    finished[(j * trials * 7) + k] = True
                if (result == 1 and board_list[(j * trials * 7) + k].player_turn != player[j]) or (
                        result == -1 and board_list[(j * trials * 7) + k].player_turn == player[j]):
                    stats[j][k // trials][1] += large_increment // moves_made
                    finished[(j * trials * 7) + k] = True
...
# simply returns one plus the index of the max value of the array; if there is a tie, the lower is selected
def pick_max(arr1):
    max_val = 0
    max_index = 0
    for i in range(len(arr1)):
