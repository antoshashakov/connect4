...
model.add(tf.keras.layers.Dense(2252, activation='sigmoid'))
...
weights_2 = [[(1 if j == 2 * i else (-1 if j == (2 * i) + 1 else 0)) for j in range(2252)] for i in range(1126)]
biases_2 = [0 for _ in range(2252)]
...
for i in range(training_cycles):
    ...
    # train the model
    history = model.fit(training, target, verbose=0, batch_size=b_size, epochs=eps)
